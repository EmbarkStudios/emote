...
from emote.models.callbacks import ModelLoss, ModelBasedCollector, LossProgressCheck
...
def test_model_collector():
    ...
    memory, dataloader = create_memory(
        env, memory_size=100, len_rollout=1, batch_size=batch_size, data_group=rl_data_group, device=device
    )
    callbacks = [
        ModelBasedCollector(
            model_env=model_env, agent=agent, memory=memory,
