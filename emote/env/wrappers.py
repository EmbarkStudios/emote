...
# Most of these wrappers are from the openai baselines repository : https://github.com/openai/baselines/blob/master/baselines/common/atari_wrappers.py
...
class ParameterisedClipRewardEnv(gym.RewardWrapper):
    ...
    def reward(self, reward):
        return np.clip(reward, self._min_clip, self._max_clip)
...
class TupeliserFrame(gym.ObservationWrapper):
    def __init__(self, env):
        ...
        if isinstance(env.observation_space, spaces.Tuple):
            self._already_tuple = True
            self.observation_space = env.observation_space
        else:
            self._already_tuple = False
            self.observation_space = spaces.Tuple((env.observation_space,))

    def observation(self, observation):
        return observation if self._already_tuple else (observation,)


class DictObservationFrame(gym.ObservationWrapper):
    def __init__(self, env):
        gym.ObservationWrapper.__init__(self, env)
        if isinstance(env.observation_space, spaces.Tuple):
            assert False, "does not deal with dict spaces"

        elif isinstance(env.observation_space, spaces.Dict):
            self._already_dict = True

        else:
            self._already_dict = False

    def observation(self, observation):
        return observation if self._already_dict else {"obs": observation}
